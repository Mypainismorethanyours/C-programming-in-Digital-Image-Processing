...
void fft(struct _complex *src, struct _complex *dst, int flag, int length) {
	NUM_x = NUM_y = length;
	int y, x, i, u, k , n;
	double wu;
...
double* getResult(struct _complex *src, int size){
	...
	for (i = 0; i < size; ++i) {
		res[i] = sqrt(src[i].x * src[i].x + src[i].y * src[i].y);// / sqrt(size);
	}

	return res;
}

double* getResult(struct _complex* src, int size,int type) {
	int i;
	double* res = (double*)malloc(sizeof(double) * size);
	for (i = 0; i < size; ++i) {
		//res[i] = 0.0;
